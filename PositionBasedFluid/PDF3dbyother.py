...
res_3D = np.array([1, 0.5, 1]) * base_size_factor * scaling_size_factor
...
old_positions = ti.Vector.field(dim, ti.f32)
positions = ti.Vector.field(dim, ti.f32)
velocities = ti.Vector.field(dim, ti.f32)
...
position_deltas = ti.Vector.field(dim, ti.f32)
# 0: x-pos, 1: timestep in sin()
board_states = ti.Vector.field(2, ti.f32)
    ...
    for p_i in positions:
        ...
        for j in range(particle_num_neighbors[p_i]):
            ...
            if p_j >= 0:
                ...
                z_grad = ti.Matrix(
                    [[0.0, vel_ij[2], -vel_ij[1]], [-vel_ij[2], 0.0, vel_ij[0]], [vel_ij[1], -vel_ij[0], 0.0]])
...
def render(gui):
    ...
    canvas.rect(ti.core_vec(0, 0),
                ti.core_vec(board_states[None][0] / boundary[0], 1.0)
...
#                                          np.array([board_states[None][0] / 5, boundary[1],
#                                                    boundary[2] / 5]) - particle_radius_in_world,
#                                          (num_particles, dim))
...
def init_particles():
    for i in range(num_particles):
        positions[i] = ti.Vector([ti.random()*boundary[0] +particle_radius_in_world,
                                 ti.random()*boundary[1] + particle_radius_in_world,
                                  ti.random()*boundary[2] + particle_radius_in_world])
        print(positions[i])
    board_states[None] = ti.Vector([boundary[0] - epsilon, -0.0])
...
def write_file(i):
    f = open(
        'D:/github/Fluid-Engine/fluid-engine-dev/out/build/x64-Release/bin/hybrid_liquid_sim_output/new_test' + str(
            i) + '.xyz', 'w')
    s = ''
    pos_np = positions.to_numpy()
    for j in range(dim):
        pos_np[:, j] *= screen_to_world_ratio / res_3D[j]
    for k in range(num_particles):
        s += str(pos_np[k][0]) + ' ' + str(pos_np[k][1]) + ' ' + str(pos_np[k][2]) + '\n'
    f.write(s)
    print('new_test' + str(i) + '.xyz has been down')
